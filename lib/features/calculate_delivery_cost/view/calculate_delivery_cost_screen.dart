...
import 'package:vasd/features/calculate_delivery_cost/bloc/calculate_delivery_cost_bloc.dart';
...
class _CalculateDeliveryCostScreenState
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<CalculateDeliveryCostBloc, CalculateDeliveryCostState>(
      listener: (context, state) {
        if (state is CalculateDeliveryCostLoadingState) {
          showDialog(
              context: context, builder: (context) => const LoadingDialog());
        } else if (state is CalculateDeliveryCostLoadedState) {
          setState(() {});
        }
      },
      listenWhen: (previous, current) {
        if (previous is CalculateDeliveryCostLoadingState) {
          Navigator.pop(context);
        }
        return true;
      },
      bloc: _calculateDeliveryCostBloc,
      child: BlocBuilder<CalculateDeliveryCostBloc, CalculateDeliveryCostState>(
        bloc: _calculateDeliveryCostBloc,
        builder: (context, state) {
          if (state is CalculateDeliveryCostLoadedState) {
            return SafeArea(
              child: Scaffold(
                appBar: AppBar(
                  leading: IconButton(
                            ...
                            content: Column(
                              ...
                              children: [
                                ...
                                Column(
                                  spacing: 12,
                                  children: [
                                    for (var variant in deliveryVariantList)
                                      typeCard(
                                        minCost:
                                            distance * variant.distanceRate,
